...
class PlaywrightExecutor:
    ...
    def navigate(self, step):
        ...
        if self.browser is None:
            raise RuntimeError(
                "Cannot navigate: browser is not open."
            )
    ...
    def find_element(self, step):
        if self.page is None:
            raise RuntimeError(
                "Cannot find element: browser is not open."
            )
        ...
        if logical_locator not in OBJECT_REPOSITORY:
            raise ValueError(
            f"Locator '{logical_locator}' "
            "not found in Object Repository."
        )

        playwright_locator = OBJECT_REPOSITORY[
        logical_locator
        ]
...
def generate_report(self):
    ...
    report_file = (
        REPORT_DIR / f"{test_name}_report.html"
    )
...
<head>
    ...
    <title>
        Test Execution Report - {safe_test_name}
    </title>
